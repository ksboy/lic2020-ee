...
class InputExample(object):
    ...
    def __init__(self, guid, id, words, labels, segment_ids):
    ...
    with open(file_path, encoding="utf-8") as f:
        for line in f:
            ...
            if len(words)!= len(labels):
                ...
                continue
            examples.append(InputExample(guid="{}-{}".format(mode, guid_index),id=id, words=words, labels=labels, \
...
def convert_examples_to_features(
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        for word, label, segment_id in zip(example.words, example.labels, example.segment_ids):
            ...
            segment_ids.extend( [sequence_a_segment_id if not segment_id else trigger_token_segment_id] * len(word_tokens))
